-- Adds the ssh-config kind, which shells out to the ssh command and lets
-- OpenSSH read ~/.ssh/config for everything.
--
-- SQLite cannot alter a CHECK constraint, so the table is rebuilt: create,
-- copy, drop, rename, then put the index back.
...
CREATE TABLE connections_new (
    ...
    kind         TEXT NOT NULL DEFAULT 'ssh'
                 CHECK (kind IN ('ssh', 'ssm', 'ssm-ssh', 'ssh-config')),
